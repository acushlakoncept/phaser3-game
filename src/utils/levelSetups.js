...
export const levelSetup = (context, gameState) => {
    ...
    gameState.goal = context.physics.add.sprite(gameState.width - 20, 120, 'door').setScale(.4);

    context.physics.add.overlap(gameState.player, gameState.goal, function() {
      context.cameras.main.fade(800, 0, 0, 0, false, function(camera, progress) {
        if (progress > .9) {
